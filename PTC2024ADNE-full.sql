...
SELECT P.DUI, CONCAT(P.nombre, ' ', P.apellido) AS 'Nombre del Profesional', P.correoElectronico, ESP.nombreEspecialidad, ESPALT.nombreEspecialidadAlt
FROM Profesional AS P
INNER JOIN Especialidad AS ESP ON P.especialidadId = ESP.especialidadId
INNER JOIN EspecialidadAlt AS ESPALT ON P.especialidadAltId = ESPALT.especialidadAltId
GO
...
--SELECT COUNT (citaId) FROM Cita WHERE estadoId = 2  -- para la opcion perdida 
--SELECT COUNT (citaId) FROM Cita WHERE estadoId = 3  -- para la opcion pendiente 
