...
--		CAMPO				TIPO DE DATO			RESTRICCIONES
		...
		profesionalId		INT						FOREIGN KEY REFERENCES Profesional(profesionalId) ON UPDATE CASCADE ON DELETE SET NULL,
...
INSERT INTO Cita (fecha, horaInicio, horaFinal, estadoId, pacienteId, profesionalId, lugarId)
VALUES
(@fecha, @horaInicio, @horaFinal, @estadoId, @pacienteId, @profesionalId, @lugarId)
GO
...
DECLARE @horaFinal			    TIME			= '13:30:00'
DECLARE @estadoId				INT				= 1
DECLARE @pacienteId				INT				= 12
DECLARE @profesionalId			INT				= 2
DECLARE @lugarId				INT				= 1
...
--		CAMPO				TIPO DE DATO			RESTRICCIONES
		...
		citaId				INT						FOREIGN KEY REFERENCES Cita(citaId) ON UPDATE CASCADE ON DELETE CASCADE
)
GO
...
SELECT C.citaId AS 'ID de la Cita', CU.consultaId AS 'ID de la Consulta', C.pacienteId AS 'ID del Paciente', C.profesionalId AS 'ID del Profesional', C.fecha AS 'Fecha de la Cita', C.horaInicio AS 'Hora de Inicio', C.horaFinal AS 'Hora de Finalización', E.estado AS 'Estado de la Cita', CU.descripcion AS 'Descripción', P.nombre AS 'Nombre del Paciente', P.apellido AS 'Apellido del Paciente', PR.nombre AS 'Profesional Encargado', L.lugar AS 'Lugar de la Cita'
FROM Consulta AS CU
...
P.pacienteId AS 'ID del Paciente', E.expedienteId AS 'ID del Expediente', C.citaId AS 'ID de la Cita'

FROM Expediente AS E
INNER JOIN Paciente AS P ON E.pacienteId = P.pacienteId
INNER JOIN Genero G ON P.generoId = G.generoId
INNER JOIN Cita C ON P.pacienteId = C.pacienteId
INNER JOIN Lugar L ON C.lugarId = L.lugarId
LEFT JOIN Consulta CO ON C.citaId = CO.citaId
GO
...
SELECT * FROM vistaObtenerInformacionYExpediente --WHERE [ID del Paciente] = @pacienteId AND [ID del Expediente] = @expedienteId AND [ID de la Cita] = @citaId
GO

CREATE VIEW vistaCita AS
SELECT C.citaId , C.pacienteId, ESP.expedienteId, E.estado AS 'Estado', CONCAT(P.nombre, ' ', P.apellido) AS 'Nombre del Paciente', C.fecha AS 'Fecha de la Cita', C.horaInicio 'Hora de Inicio'
FROM Cita AS C
INNER JOIN Paciente AS P ON C.pacienteId = P.pacienteId
INNER JOIN Estado AS E ON C.estadoId = E.estadoId
INNER JOIN Expediente AS ESP ON P.pacienteId = ESP.pacienteId
GO
